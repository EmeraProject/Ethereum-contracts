...
contract('GEMERAToken', function (accounts) {
  ...
  it('titles', async function () {
    ...
    decimals.should.be.bignumber.equal(8);
  });

  it('restricted', async function () {
    await this.token.mint(accounts[0], 3000, { from: accounts[0] }).should.be.fulfilled;
  ...
  it('burnable list', async function () {
    await this.token.burn(500, { from: accounts[0] }).should.be.rejectedWith(EVMRevert);
    ...
    await this.token.mint(burnableList[0], 1500, { from: accounts[0] });
  ...
  it('ownable approved', async function () {
    ...
    await this.token.transferOwnership(accounts[1], { from: accounts[0] }).should.be.fulfilled;
    assert.equal(await this.token.owner(), accounts[0]);
